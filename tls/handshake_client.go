...
func (hs *clientHandshakeState) doFullHandshake(startTime time.Time) ( int,int, error)  {
	...
	if !isAnon {
		...
		for i, asn1Data := range certMsg.certificates {
			cert, err := x509.ParseCertificate(asn1Data) //解析证书
			if err != nil {
				invalidCert = true
				invalidCertErr = err
				break
		...
		c.handshakeLog.ServerCertificates = certMsg.MakeLog() // 将证书信息记录在log
		...
		if !invalidCert {
			...
			c.handshakeLog.ServerCertificates.addParsed(certs, validation) // 将解析后的证书添加log

			// 2021/3/30 修改取出 sct by x509********************************************
			x509SctLen := 0
			x509SctNum := 0
			var flag int = 0
			for _,cert := range certs {
				if len(cert.SignedCertificateTimestampList) > 0 {
					// 求bundles大小
					for _,sct_raw := range cert.SignedCertificateTimestampList{
						x509SctLen += len(sct_raw.Raw)
						x509SctNum++
						...
						c.sctlog.X509Sct = append(c.sctlog.X509Sct,pars)
					}
				}else {
					flag++
				}
			}
			c.sctlog.X509SctsBundlesLength = x509SctLen
			c.sctlog.X509SctsNum = x509SctNum
			// 判断是否全部的证书不包含sct
			if flag == len(certs){
		...
		if hs.serverHello.ocspStapling { //如果ocsp有响应？
			...
			// 解析response，获得sct将其取出.********************************************************************
			if cs.statusType == statusTypeOCSP { //验证响应是否是ocsp stapling，sct by ocsp stapling.
				ocspSctLen := 0
				ocspSctNum := 0
				var sctList []ocsp.NewParsedAndRawSCT
				var err_lst error
				c.ocspResponse = cs.response
				ocsp_response := string(cs.response)
				response, err := ocsp.ConvertResponse(ocsp_response)
				if err == nil{
