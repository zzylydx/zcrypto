...
// 2021/3/30 SignedCertificateTimestampLog--->SCTLog*******************************************
...
// 2021/4/1 插入log**************************************************************************
func (c *Conn) GetsctLog() *SignedCertificateTimestampLog {
	return c.sctlog
}
// *****************************************************************************************
...
func (m *serverHelloMsg) MakeLog() *ServerHello {
	...
	if len(m.scts) > 0 {
		for _, rawSCT := range m.scts {
			...
			copy(out.Raw, rawSCT)
			sct, err := ct.DeserializeSCT(bytes.NewReader(rawSCT)) //解析sct
			if err == nil {
				out.Parsed = sct
			}
...
// chain slice has already been allocated. 将解析后的证书添加到log
