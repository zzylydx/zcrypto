...
type Certificate struct {
	...
	// CT
	SignedCertificateTimestampList []*ParsedAndRawSCT // sct by cert,修改了原来为：[]*ct.signedcertificatetimestamp
	...
	for _, e := range in.TBSCertificate.Extensions {
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			for len(scts) > 0 {
				...
				pars.Parsed = sct
				scts = scts[2+length:]
				out.SignedCertificateTimestampList = append(out.SignedCertificateTimestampList, pars)
				// ***********************
			}
